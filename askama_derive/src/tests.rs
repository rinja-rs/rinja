    ...
    let expected: syn::File = syn::parse_quote! {
        impl askama::Template for Foo {
            fn render_into_with_values<RinjaW>(
                &self,
                __askama_writer: &mut RinjaW,
                __askama_values: &dyn askama::Values,
            ) -> askama::Result<()>
            where
                RinjaW: askama::helpers::core::fmt::Write + ?askama::helpers::core::marker::Sized,
            {
        ...
        impl askama::filters::FastWritable for Foo {
            #[inline]
            fn write_into<RinjaW>(&self, dest: &mut RinjaW) -> askama::Result<()>
            where
                RinjaW: askama::helpers::core::fmt::Write + ?askama::helpers::core::marker::Sized,
            {
