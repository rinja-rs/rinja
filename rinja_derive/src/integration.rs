...
use syn::{
    Data, DeriveInput, Fields, GenericParam, Generics, Ident, Lifetime, LifetimeParam, Member,
    Token, Type, Variant, parse_quote,
};

use crate::generator::TmplKind;
use crate::input::{PartialTemplateArgs, TemplateArgs};
use crate::{CompileError, build_template_struct};
...
pub(crate) fn build_template_enum(
    ...
    for (var, var_args) in enum_data.variants.iter().zip(vars_args) {
        ...
        let size_hint = biggest_size_hint.max(build_template_struct(
            buf,
            &var_ast,
    ...
    if has_default_impl {
        let size_hint = build_template_struct(
            buf,
            enum_ast,
        ...
        render_into_arms.extend(quote! {
            ref __rinja_arg => {
                <_ as rinja::helpers::EnumVariantTemplate>::render_into(__rinja_arg, writer)
            }
        });
    ...
    buf.write(format_args!(
        ...
        \
        fn render(&self) -> rinja::Result<rinja::helpers::std::string::String> {{\
            let size_hint = match self {{\
                {size_hint_arms}\
            }};\
            let mut buf = rinja::helpers::std::string::String::new();\
...
fn variant_as_arm(
    ...
    match &var.fields {
        Fields::Named(fields) => {
            ...
            let Member::Named(phantom) = ast_data.fields.members().last().unwrap() else {
                unreachable!();
            };
    ...
    render_into_arms.extend(quote! {
        Self :: #ident { #src } => {
            <_ as rinja::helpers::EnumVariantTemplate>::render_into(
