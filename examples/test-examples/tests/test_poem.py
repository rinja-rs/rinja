...
class TestWebApps(TestCase):
    @with_server_and_session("poem")
    def test_poem(self, server: PopenSpawn, session: Session):
        server.expect("server started", timeout=10.0)
        sleep(0.1)
        ...
        with session.get("http://127.0.0.1:8080/en/greet-me.html") as resp:
            ...
            self.assertEqual(resp.text, read("poem/en_greet-me.html"))
