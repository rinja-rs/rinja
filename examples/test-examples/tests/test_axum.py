...
class TestWebApps(TestCase):
    @with_server_and_session("axum")
    def test_axum(self, server: PopenSpawn, session: Session):
        server.expect("Listening on http://127.0.0.1:8080/", timeout=10.0)
        sleep(0.1)
        ...
        with session.get("http://127.0.0.1:8080/en/greet-me.html") as resp:
            ...
            self.assertEqual(resp.text, read("axum/en_greet-me.html"))
